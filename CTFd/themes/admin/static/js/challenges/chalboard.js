...
function load_chal_template(id, success_cb){
    ...
    $.get(script_root + "/admin/chal/" + id, function (challenge_data) {
        $.get(script_root + obj.type_data.templates.update, function (template_data) {
            ...
            var pageNo = currentPath.substr(currentPath.lastIndexOf('/')+1);
            if(isNaN(pageNo)){
                pageNo = 1;
            }
            $("#update-modals-entry-div form").attr("action","/admin/chal/update/" + pageNo);
...
function loadchals(cb){
    ...
    $.post(script_root + "/admin/chals/" + pageNo, {
        'nonce': $('#nonce').val()
    }, function (data) {
        var categories = [];
