...
from sqlalchemy.sql import or_, distinct, func
...
@challenges.route('/chals_ajax/<category>', methods=['GET'])
@during_ctf_time_only
@require_verified_emails
@viewable_without_authentication(status_code=403)
def chals_ajax(category):
    db_chals = Challenges.query.filter(or_(Challenges.hidden != True, Challenges.hidden == None)).filter(Challenges.category == category).order_by(Challenges.value).all()
